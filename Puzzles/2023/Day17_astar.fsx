...
let solve1 input =
    ...
    let config: Config<State> = {
        fCost = (fun s1 s2 -> 1) //manhattanDistance s1.row s1.col s2.row s2.col)
...
let solve2 input =
    ...
    let start = { row = 0; col = 0; dir = None; straight = 0 }
    let target = { row = grid |> maxR; col = grid |> maxC; dir = None; straight = 0 }
    match searchWithGoalFunc start target (fun s -> s.row = (grid |> maxR) && s.col = (grid |> maxC) && s.straight >= 4) config with
    | Some cells -> 
        let totalCost = 
            cells 
            |> Seq.filter (fun s -> s.row <> 0 || s.col <> 0)
...
if solve2 "Day17_sample1.txt" <> 94 then failwithf "Sample 1 not passing"
if solve2 "Day17_sample2.txt" <> 71 then failwithf "Sample 2 not passing"
